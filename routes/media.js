...
exports.browse = function(req, res){
	...
	request(getBrowseUrl(newTime, offset, length, category), function(error, response, body){
		if(!error && response.statusCode == 200){			
			...
			json.next = req.protocol + "://" + req.get("host") + req.path + "?" + (category ? ("category=" + category) : "") + "&time=" + time + "&length=" + length + "&offset=" + (parseInt(offset) + length);
